...
void rc522_regWrite8(uint8_t reg, uint8_t data8); //Écriture d’un bloc
...
/*
	* reset logiciel du MFRC522
	=> écrit 0x0F dans CommandReg (adresse 0x01) — PCD_RESETPHASE.
*/
void rc522_reset(void);

/*
	*activer l’antenne RF interne.
	=>lit TX_CONTROL, si bits 0x03 non positionnés alors les met (setBit).
*/
...
bool rc522_antiColl(uint8_t* serNum); //// Anti-collision et lecture UID

/**
  * @brief Halt ( sleep mode )
	* Construit commande HALT (PICC_HALT), calcule CRC via rc522_calculateCRC et envoie en PCD_TRANSCEIVE
 */
...
/**
	* @brief compare IDs :Compare 4 octets d’UID.
 */
