/*
 * Driver MFRC522 (RC522) 
 * Ce fichier fournit :
 *  - lectures/ecritures de registres MFRC522 via SPI
 *  - commandes haut-niveau (request, anticollision, halt)
 *  - utilitaires (CRC interne, set/clear bits, activation antenne)

 *  - Pinout: PA5=SCK, PA6=MISO, PA7=MOSI, PB0=CS, PA8=RST
 */
 ...
 * �crit 0x0F dans le registre Command (adresse 0x01) reset interne du MFRC522.
 */
...
void rc522_antennaON(void)
{
  ...
  if (!(temp & 0x03)) {
	...
	/*
		* envoie REQ pour d�tecter carte
		* Si ok, rc522_antiColl(id) pour r�cup�rer UID (4 octets)
		  rc522_halt() pour mettre la carte en sleep.
		  Retourne bool succ�s / �chec.
	*/
...
/*
	* Ecrit MFRC522_REG_BIT_FRAMING = 0x07 puis pr�pare la commande (tagType[0]=reqMode) et invoque rc522_toCard avec PCD_TRANSCEIVE.
  * V�rifie que backBits == 0x10 (taille attendue), sinon �chec.
*/
bool rc522_request(uint8_t reqMode, uint8_t *tagType)
{
...
/*
(Senario :
	controle d'acces V�rifier si la carte d�tect�e correspond � une carte autoris�e (contr�le d'acc�s). ) 
	* compare octet par octet deux tableaux d'UID et renvoie true 
	* si tous les 4 octets sont identiques, false sinon.
*/
...
void RC522_SPI_Init(void)
{
  ...
  rc522_antennaON();   //Activer l'antenne
}